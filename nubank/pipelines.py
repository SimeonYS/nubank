...
class NnubankPipeline:

    # Database setup
    conn = sqlite3.connect('nubank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `nubank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM nubank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO nubank (date, title, link, content)"
